...
class MessagesController extends Controller
{
    ...
    // Contoh pada MessagesController.php
public function startChat($id_pengguna_produk)
{
    // Temukan pengguna yang mengupload produk
    $userTo = User::find($id_pengguna_produk);

    // Validasi jika pengguna ditemukan
    if (!$userTo) {
        return redirect()->route('produk.index')->with('error', 'Pengguna tidak ditemukan.');
    }

    // ID pengguna yang mengirim permintaan chat
    $userFromId = Auth::user()->id;

    // Buka obrolan atau temukan obrolan yang sudah ada antara dua pengguna ini
    $chat = Chatify::getChat($userFromId, $userTo->id);

    // Jika obrolan belum ada, buat obrolan baru
    if (!$chat) {
        $chatId = Chatify::createNewChat($userFromId, $userTo->id);
    } else {
        $chatId = $chat->id;
    }

    // Redirect pengguna ke halaman obrolan dengan ID obrolan yang baru atau yang sudah ada
